...
const wordCounter = Vue.createApp({
    data(){
        return{
            inpCount: 0,
        }
...
const changeBGonHover = Vue.createApp({
    data() {
        return {
            colorVal: 50,
            backgroundColor: 'hsl(50, 60%, 60%)'
        };
    },
    methods: {
        changeColor() {
            this.colorVal = Math.floor(Math.random() * 360);
            this.backgroundColor = `hsl(${this.colorVal}, 60%, 60%)`;
        }
    }
});
...
wordCounter.mount('#countData')
changeBGonHover.mount('#vue-events')
